  ...
  <view class="container bg-white w-100 h-100">
    <view style="padding-bottom: 220rpx">
      ...
      >
        ...
        <button type="primary" class="b" @tap="recharge">购买</button>
      </view>
      <!-- bottom box end -->
    </view>
  </view>
...
export default Vue.extend({
  ...
  computed: {
    ...mapGetters(['userInfo']),
    // 使用说明
    rechargeCard(): unknown {
      return (
        (this as any).amounts.find((item: unknown) => (item as any).selected) ||
        ""
      );
    },
  },
  async onLoad() {
    const amounts = await (this as any).$api("rechargeCards");
    // 每次打开恢复第一项选中
    amounts.forEach((item: any, index: number) => {
      if (index === 0) {
  ...
  methods: {
    ...
    recharge() {
      ...
      setTimeout(() => {
        this.setAddBalance(this.balance);
        uni.showToast({
          title: "充值成功",
          icon: "success",
        });
