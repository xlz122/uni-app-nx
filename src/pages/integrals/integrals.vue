...
export default Vue.extend({
  ...
  async onLoad() {
    ...
    this.customPoints = await (this as any).$api("customPoints");
  ...
  methods: {
    ...
    async getStepsOptions() {
      ...
      attendance.forEach((item: any, index: number) => {
        ...
        if (index == attendance.length - 1) {
          (arr as any).circle = "/static/images/integrals/goal.png";
          (arr as any).circleStyle = "width: 47rpx; height: 39rpx;";
        }

        stepsOptions.push(arr);
      });

      (this as any).stepsOption = stepsOptions;
		},
		// 获取积分商品列表
    async getPointsMall() {
    ...
    flow(): void {
      uni.navigateTo({
		...
		// 积分商品详情
    detail(cate: number, id: number): void {
      uni.navigateTo({
